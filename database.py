...
class QuizDatabase:
        ...
        with self.get_locked_cursor() as cursor:
            ...
            if not batch_config:
                ...
                if not batch_config:
                    ...
                    # 获取默认批次大小
                    cursor.execute(
                        "SELECT default_batch_size FROM domain WHERE id = ?",
                        (domain_id,),
                    )
                    row = cursor.fetchone()
                    limit = row["default_batch_size"] if row else 3

                    logger.warning(
                        f"No domain_settings for domain {domain_id}, using fallback limit={limit}"
                    )
                    problems = self.get_problems_for_push(domain_id, limit=limit)
    ...
    def get_cursor_record(self, group_qq: str, domain_id: int) -> dict | None:
        """
        检查 cursor 记录是否存在
        ...
        Returns:
            dict: cursor 记录，包含 now_cursor, push_time 等字段
            None: 记录不存在
        """
        with self.get_locked_cursor() as cursor:
            cursor.execute(
    ...
    def init_cursor(self, group_qq: str, domain_id: int, push_time: str = "17:00"):
        """
        初始化 cursor 记录
